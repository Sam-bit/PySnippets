...
def get_market_cap(symbol):
    try:
        market_cap_data = int(web.get_quote_yahoo(symbol).iloc[31])
    except:
        return 0
...
for i in range(2, my_row + 1):
    ...
    oneyearhigh_price = get_52weekshigh_price(
        str(symbol_cell.value) + "." + ("NS" if exchange_cell.value == "NSE" else "BO"))
    oneyearlow_price = get_52weekslow_price(
        str(symbol_cell.value) + "." + ("NS" if exchange_cell.value == "NSE" else "BO"))
    ...
    my_sheet_obj.cell(row=i, column=27).value = ex_dividend_date
    my_sheet_obj.cell(row=i, column=28).value = ex_dividend_amt_yield
